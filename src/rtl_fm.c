...
static void socket_set_demod (long data)
{
	switch (data)
	{
		case 0:
...
static void *socket_thread_fn(void *arg)
{
    ...
    socklen_t clilen;
    unsigned char buffer[UDP_BUFFSIZE];
	...
	while (!do_exit)
	{
		while((n = read(sockfd,buffer,UDP_BUFFSIZE)) != 0) 
		{

			packet.cmd = buffer[0];
			memcpy (&packet.data, buffer+1, sizeof(packet.data));
			fprintf (stderr, "Cmd: %i", packet.cmd);
			fprintf (stderr, " Data: %li\n", packet.data);
			
			switch (packet.cmd)
			{
				case 0:
					new_freq = packet.data;
					fprintf (stderr, "Tuning to: %d [Hz] (central freq: %d [Hz])\n", new_freq, new_freq + freq_offset);
					rtlsdr_set_center_freq(dongle.dev, new_freq + freq_offset);
					break;
				case 1:
					socket_set_demod (packet.data);
					break;
				default:
					break;
			}
...
int main(int argc, char **argv)
{
	...
	pthread_create(&socket_freq, NULL, socket_thread_fn, NULL);
	if (output.lrmix) {
